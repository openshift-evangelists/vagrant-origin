...
Vagrant.configure(2) do |config|
   ...
   config.vm.provision :shell, :path => "./scripts/prerequisites.sh", :args => "#{FORCE_PREREQS}"
  
   # Setup the VM
   config.vm.provision :shell, :path => "./scripts/configure_docker.sh", :args => "#{FORCE_DOCKER}"

   # Build Origin
   config.vm.provision :shell, :path => "./scripts/build_origin.sh", :args => "#{PUBLIC_ADDRESS} #{PUBLIC_DOMAIN} #{ACTION} #{ORIGIN_REPO} #{ORIGIN_BRANCH}"
